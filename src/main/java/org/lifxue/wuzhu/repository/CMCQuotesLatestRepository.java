 ...
 */
@Repository
public interface CMCQuotesLatestRepository extends JpaRepository<CMCQuotesLatest, Integer> {
    ...
    @Query(value = "SELECT * FROM CMC_QUOTES_LATEST a inner join (SELECT MAX(id) AS max_id FROM CMC_QUOTES_LATEST GROUP BY TID) as b on a.ID=b.max_id", nativeQuery=true)
